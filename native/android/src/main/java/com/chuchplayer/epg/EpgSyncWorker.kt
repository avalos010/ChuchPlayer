    ...
    private fun getRealmInstance(): Realm {
        ...
        return try {
            ...
            if (errorMessage.contains("unsupported version", ignoreCase = true) || 
                ...
                android.util.Log.w(TAG, "Database version incompatible, deleting and recreating: $errorMessage")
                try {
                    Realm.deleteRealm(config)
                    android.util.Log.d(TAG, "Deleted incompatible database, recreating...")
                    // Try to get instance again after deletion
                    Realm.getInstance(config)
                } catch (deleteError: Exception) {
                    android.util.Log.e(TAG, "Failed to delete/recreate database: ${deleteError.message}", deleteError)
                    ...
                    try {
                        ...
                        android.util.Log.e(TAG, "All recovery attempts failed: ${fallbackError.message}", fallbackError)
                        throw e
                    }
                }
            } else {
            ...
            android.util.Log.e(TAG, "Unexpected error getting Realm instance: ${e.message}", e)
            try {
                Realm.deleteRealm(config)
                android.util.Log.d(TAG, "Deleted database due to error, recreating...")
                Realm.getInstance(config)
            } catch (recoveryError: Exception) {
                android.util.Log.e(TAG, "Failed to recover from error: ${recoveryError.message}", recoveryError)
                throw e
            }
        }
    }
